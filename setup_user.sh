...
yes_or_no "Install Fail2ban? (Anti SSH Brute Force)" && sudo apt-get install fail2ban -y && sudo cp /etc/fail2ban/jail.conf /etc/fail2ban/jail.local && sudo sed -i "s/port    = ssh\b/port    = ssh \nfilter = sshd \nmaxretry = 5 \nenabled = true \nmode = normal \nbantime = 600 \nfindtime = 600 \naction = ufw/gI" /etc/fail2ban/jail.local && sudo sed -i "s/logpath =.*/logpath = \/var\/log\/auth.log/gI" /etc/fail2ban/jail.local && sudo sed -i "s/backend = .*/backend = systemd/gI" /etc/fail2ban/jail.local && sudo systemctl restart fail2ban && sudo systemctl status fail2ban #&& \
