...
yes_or_no "Install Fail2ban? (Anti SSH Brute Force)" && apt-get install fail2ban -y && sudo cp /etc/fail2ban/jail.conf /etc/fail2ban/jail.local && sudo sed -i "s/port    = ssh\b/port    = ssh \nfilter = sshd \nmaxretry = 5 \nenabled = true \nmode = normal/gI" /etc/fail2ban/jail.local && sudo sed -i "s/logpath = %(sshd_log)s\b/logpath = /var/log/auth.log/gI" /etc/fail2ban/jail.local && sudo sed -i "s/backend = %(sshd_backend)s\b/backend = systemd/gI" /etc/fail2ban/jail.local && sudo systemctl restart fail2ban && sudo systemctl status fail2ban #&& \
